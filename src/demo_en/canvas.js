...
export class CanvasManager {
  ...
  enterConnectionMode(sourceNode = null) {
    ...
    if (this.connectionIndicator) {
      this.connectionIndicator.textContent = sourceNode
        ? `Connect from "${sourceNode.text.substring(0, 30)}..." — click the target`
        : 'Connection mode — click the source';
  ...
  handleConnectionClick(node) {
    if (!this.interaction.connectionSource) {
      ...
      if (this.connectionIndicator) {
        this.connectionIndicator.textContent = `Connect from "${node.text.substring(0, 30)}..." — click the target`;
      }
