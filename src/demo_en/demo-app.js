 ...
 * and runs simulated DEVELOP / CONNECT.
 */
...
class DemoApp {
  ...
  setupMetricsHooks() {
    // Keep originals for use during batch operations (DEVELOP/CONNECT)
    ...
    this.canvas.createNode = (...args) => {
      this.history.saveState('create card');
    ...
    this.canvas.deleteNode = (...args) => {
      this.history.saveState('delete card');
    ...
    this.canvas.createConnection = (...args) => {
      this.history.saveState('create connection');
    ...
    this.canvas.deleteConnection = (...args) => {
      this.history.saveState('delete connection');
    ...
    this.canvas.cycleNodeStatus = (...args) => {
      this.history.saveState('change status');
    ...
    this.canvas.deleteSelectedNodes = (...args) => {
      this.history.saveState('delete selection');
    ...
    document.addEventListener('nodeDragStart', () => {
      if (!dragSaved) {
        this.history.saveState('move card');
        dragSaved = true;
      }
    });
  ...
  updateMetrics() {
    ...
    if (total === 0) {
      texte.textContent = 'Empty canvas — create your first card with + Card.';
    } else if (total < 3) {
      texte.textContent = `${total} card(s) — add ideas then launch DEVELOP.`;
    } else if (isolated > total * 0.3) {
      texte.textContent = `${total} cards, ${isolated} orphan(s) — launch CONNECT to link them.`;
    } else {
      texte.textContent = `${total} cards, ${connections.length} connections`;
    }
  ...
  setupToolbar() {
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', () => {
        ...
        this.history.saveState('clear canvas');
        this.canvas.clear();
        this.updateMetrics();
      });
    }
  ...
  async runDevelopper() {
    ...
    if (this.canvas.state.nodes.length === 0) {
      this.showBandeauMessage('Add at least one card before launching DEVELOP.');
      return;
    }

    const btn = document.getElementById('btn-developper');
    this.setOperationLoading(btn, true, 'Thinking...');

    // Save state for undo
    this.history.saveState('DEVELOP');
    ...
    this.updateMetrics();
    this.setOperationLoading(btn, false, 'DEVELOP');
  }

  async runRelier() {
    if (this.isOperationRunning) return;
    if (this.canvas.state.nodes.length < 2) {
      this.showBandeauMessage('You need at least 2 cards to launch CONNECT.');
      return;
    }

    const btn = document.getElementById('btn-relier');
    this.setOperationLoading(btn, true, 'Analyzing...');

    this.history.saveState('CONNECT');
    ...
    this.canvas.renderAllConnections();
    this.updateMetrics();
    this.setOperationLoading(btn, false, 'CONNECT');
  }
