...
export function showNodeMenu(cm, node, x, y) {
  ...
  const items = [
    { label: '\u270F\uFE0F Edit', action: () => { closeActiveMenu(); cm.showEditModal(node); } },
    { label: '\uD83D\uDD17 Connect (L)', action: () => { closeActiveMenu(); cm.enterConnectionMode(node); } },
    { label: node.status === 'priority' ? '\u25CB Remove anchor' : '\uD83C\uDFAF Set as anchor',
      action: () => { closeActiveMenu(); cm.cycleNodeStatus(node); } },
    { separator: true },
    { label: '\uD83D\uDDD1\uFE0F Delete', danger: true, action: () => { closeActiveMenu(); cm.deleteNode(node); } },
  ];
...
export function showConnectionMenu(cm, conn, x, y) {
  ...
  const items = [
    ...
    { label: `Switch to ${otherLabel}`, action: () => {
      closeActiveMenu();
      conn.type = otherType;
      cm.renderAllConnections();
    }},
    { separator: true },
    { label: '\uD83D\uDDD1\uFE0F Delete', danger: true, action: () => {
      closeActiveMenu();
...
export function showConnectionTypeMenu(cm, fromNode, toNode, x, y) {
  ...
  const types = [
    { type: 'implies', symbol: '\u2192', label: 'Implies', desc: 'A leads to B' },
    { type: 'resonance', symbol: '\u2194', label: 'Resonance', desc: 'A and B echo each other' },
  ];
